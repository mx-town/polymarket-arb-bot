...
@dataclass
class EngineConfig:
    """Engine configuration for signal evaluation."""
    ...
    dutch_book_threshold: float = 0.99
...
def build_config(args: argparse.Namespace | None = None) -> BotConfig:
    ...
    if "engine" in yaml_config:
        engine_yaml = yaml_config["engine"]
        config.engine = EngineConfig(
            surface_path=engine_yaml.get("surface_path", "research/models/probability_surface.json"),
            use_model=engine_yaml.get("use_model", True),
            min_edge_threshold=engine_yaml.get("min_edge_threshold", 0.03),
            min_confidence_score=engine_yaml.get("min_confidence_score", 0.5),
            require_reliable=engine_yaml.get("require_reliable", True),
            use_conservative_edge=engine_yaml.get("use_conservative_edge", True),
            dutch_book_threshold=engine_yaml.get("dutch_book_threshold", 0.99),
        )
