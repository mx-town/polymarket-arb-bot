...
class PipelineJobStatus(BaseModel):
    ...
    command: str = Field(
        description="CLI command being executed (init, rebuild, observe, verify, analyse)"
    )
    ...
    progress: list[PipelineProgressEvent] = Field(
        default_factory=list, description="Stage progress trail"
    )
    exit_code: int | None = Field(
        default=None, description="Process exit code (None if still running)"
    )
...
class PipelineStatus(BaseModel):
    ...
    model_age_hours: float | None = Field(
        default=None, description="Hours since model was last modified"
    )
    ...
    observation_size_mb: float = Field(
        default=0, description="Total size of observation files in MB"
    )
    current_job: PipelineJobStatus | None = Field(
...
class PipelineStartRequest(BaseModel):
    ...
    args: dict = Field(
        default_factory=dict, description="Command arguments (e.g., {months: 6, duration: 3600})"
    )
