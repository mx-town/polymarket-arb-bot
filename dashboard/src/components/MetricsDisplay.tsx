...
export function MetricsDisplay({ metrics, connected }: Props) {
  ...
  const winRate = metrics.trades_filled > 0
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <Stat label="Daily" value={formatPnl(metrics.daily_pnl)} accent={metrics.daily_pnl >= 0 ? 'green' : 'red'} size="sm" />
      ...
      <div style={{
        ...
        {/* Status Card */}
        <MetricCard title="Status" icon="◉" accent={connected ? 'var(--accent-green)' : 'var(--accent-red)'}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
              ...
              <Stat label="Latency" value={`${metrics.avg_latency_ms.toFixed(0)}ms`} size="sm" accent={metrics.avg_latency_ms > 500 ? 'amber' : undefined} />
            </div>
          </div>
        </MetricCard>
        ...
        <MetricCard title="Activity" icon="⚡" accent="var(--accent-blue)">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
            ...
            <Stat
              label="Hit Rate"
              value={metrics.opportunities_seen > 0 ? `${(metrics.opportunities_taken / metrics.opportunities_seen * 100).toFixed(0)}%` : '—'}
              size="sm"
            />
          </div>
        </MetricCard>
        ...
        <MetricCard title="Execution" icon="↗" accent="var(--accent-amber)">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
            ...
            <Stat label="Failed" value={metrics.trades_failed} accent={metrics.trades_failed > 0 ? 'red' : undefined} size="sm" />
            <Stat label="Reconnects" value={metrics.ws_reconnects} accent={metrics.ws_reconnects > 5 ? 'amber' : undefined} size="sm" />
            <Stat label="Losses" value={`${metrics.consecutive_losses} streak`} accent={metrics.consecutive_losses > 2 ? 'red' : undefined} size="sm" />
          </div>
        </MetricCard>
      </div>
    </div>
