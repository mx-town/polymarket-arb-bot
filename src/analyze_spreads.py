...
def fetch_markets_with_clob(limit: int = 100) -> list[MarketData]:
    ...
    print(f"  Fetching CLOB order books...")
...
def print_report(markets: list[MarketData], top_n: int = 20):
    ...
    for m in balanced[:top_n]:
        ...
        print(f" {m.midpoint_sum:>6.3f} {m.yes_bid:>6.2f} {m.yes_ask:>6.2f} {m.no_bid:>6.2f} {m.no_ask:>6.2f} {liq_str:>7}  {m.question}")
    
    # Check for actual arb (total_ask < 1.0)
    arb_opps = [m for m in markets if m.total_ask < 1.0]
    
    print(f"\n{'─'*90}")
    print(f" ARB OPPORTUNITIES (YES_ask + NO_ask < $1.00)")
    print(f"{'─'*90}")
    ...
    if arb_opps:
        ...
        print(f" None found. Market is efficient.")
    ...
    print(f" ")
    print(f" Markets within 2% of efficient: {tight_markets} ({tight_markets/len(markets)*100:.1f}%)")
    print(f" Markets with arb opportunity:   {len(arb_opps)}")
    
    print(f"\n{'─'*90}")
    print(f" INTERPRETATION")
    print(f"{'─'*90}")
    print(f" • Midpoint sum ≈ $1.00 means balanced, liquid market")
    print(f" • Midpoint sum >> $1.00 means one-sided market with no real NO liquidity")
    print(f" • Arb exists only when YES_ask + NO_ask < $1.00 (rare, transient)")
    print(f" • To catch arb: use WebSocket for real-time price streaming")
    print(f"{'═'*90}\n")
