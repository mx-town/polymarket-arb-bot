"""SQLAlchemy Core table definitions for dashboard persistence.

All timestamps are Unix epoch floats — works identically in SQLite and PostgreSQL.
Designed for PostgreSQL migration: swap connection string, tables work as-is.
"""
...
btc_prices = Table(
    ...
    Column("range_pct", Float),
    Column("session_id", String(50)),
    Index("ix_btc_prices_ts", "ts"),
)
...
probability_snapshots = Table(
    ...
    Column("id", Integer, primary_key=True, autoincrement=True),
    Column("ts", Float, nullable=False),
    Column("market_slug", String(120), nullable=False),
    Column("up_bid", Float),
    Column("up_ask", Float),
    Column("down_bid", Float),
    Column("down_ask", Float),
    Column("edge", Float),
    Column("up_bid_size", Float),
    Column("up_ask_size", Float),
    Column("down_bid_size", Float),
    Column("down_ask_size", Float),
    Column("session_id", String(50)),
    Index("ix_prob_ts_slug", "ts", "market_slug"),
)
