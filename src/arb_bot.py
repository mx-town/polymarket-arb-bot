...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--min-net-profit", type=float, default=0.005,
...
class PolymarketArbBot:
        ...
        for i in range(0, total, batch_size):
            ...
            try:
                ...
                for m in batch:
                    ...
                    # Check thresholds
                    if gross_profit >= self.config.min_spread and net_profit >= self.config.min_net_profit:
                        opp = ArbOpportunity(
    ...
    def execute_arbitrage(self, opp: ArbOpportunity) -> bool:
        ...
        if self.config.dry_run:
            ...
            logger.info(f"   YES @ ${opp.yes_price:.4f} + NO @ ${opp.no_price:.4f} = ${opp.total_cost:.4f}")
            logger.info(f"   Gross: ${opp.gross_profit:.4f} ({opp.gross_profit*100:.2f}%)")
            logger.info(f"   Net:   ${opp.net_profit:.4f} ({opp.net_profit*100:.2f}%) after ~{POLY_FEE_RATE*100:.1f}% fees")
            return True
        ...
        try:
            ...
            yes_order = self.client.create_order(OrderArgs(
                price=opp.yes_price, size=shares, side=BUY, token_id=opp.yes_token_id
            ))
            self.client.post_order(yes_order)
            
            no_order = self.client.create_order(OrderArgs(
                price=opp.no_price, size=shares, side=BUY, token_id=opp.no_token_id
            ))
    ...
    def run_cycle(self):
        ...
        if opportunities:
            ...
            logger.info(f"  No opportunities (need >{self.config.min_spread*100:.1f}% gross, >{self.config.min_net_profit*100:.2f}% net)")
