...
from rebate_maker.persistence.schema import metadata
...
def cleanup_old_data(retention_days: int = 30) -> int:
    ...
    import time
    from rebate_maker.persistence.schema import pnl_snapshots
    ...
    with engine.begin() as conn:
        result = conn.execute(
            pnl_snapshots.delete().where(pnl_snapshots.c.ts < cutoff)
        )
