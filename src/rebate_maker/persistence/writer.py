...
from rebate_maker.persistence.db import get_engine
from rebate_maker.persistence.schema import (
    market_windows,
    pnl_snapshots,
    trades,
)

log = logging.getLogger("rm.persistence.writer")
...
class BatchWriter:
    def __init__(self, session_id: str):
        ...
        self._pending_updates: list[tuple[Table, dict, str]] = []
    ...
    def _event_to_rows(self, event: Event) -> list[tuple[Table, dict]]:
        ...
        if t in (EventType.ORDER_PLACED, EventType.ORDER_FILLED, EventType.ORDER_CANCELLED):
            return [(trades, {
                ...
                "strategy": d.get("strategy", "rebate_maker"),
            })]
