...
from shared.models import GabagoolMarket, TopOfBook

log = logging.getLogger("shared.market_data")
...
def _candidate_5m_slugs(asset_prefix: str, now_epoch: float) -> list[str]:
    """Generate candidate slugs for 5m updown markets.

    Format: {asset}-updown-5m-{epoch} where epoch aligns to 300s boundaries.
    """
    now_sec = int(now_epoch)
    start = (now_sec // 300) * 300
    # Cover current + next + previous window
    return [
        f"{asset_prefix}-updown-5m-{epoch}"
        for epoch in range(start - 300, start + 600 + 1, 300)
    ]
...
def _fetch_market_by_slug(slug: str) -> Optional[GabagoolMarket]:
    ...
    try:
        ...
        if "updown-5m" in event_slug:
            market_type = "updown-5m"
        elif "updown-15m" in event_slug:
...
def _parse_end_time(event: dict, slug: str, market_type: str) -> Optional[float]:
    ...
    # Fallback: parse from slug for 5m/15m markets
    if market_type == "updown-5m":
        parts = slug.split("-")
        if len(parts) >= 4:
            try:
                epoch = int(parts[-1])
                return float(epoch + 300)  # +5 minutes
            except ValueError:
                pass
    elif market_type == "updown-15m":
        parts = slug.split("-")
        if len(parts) >= 4:
            try:
                epoch = int(parts[-1])
...
def discover_markets(assets: tuple[str, ...]) -> list[GabagoolMarket]:
    ...
    for asset in assets:
        # 5m markets
        prefix_5m = _ASSET_PREFIXES_5M.get(asset)
        if prefix_5m:
            for slug in _candidate_5m_slugs(prefix_5m, now):
